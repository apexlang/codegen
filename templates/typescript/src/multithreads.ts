...
if (cluster.isMaster) {
  console.log(
    `Primary ${process.pid} is in control running with ${numCPUs} threads`,
  );
  ...
  cluster.on("disconnect", (worker: Worker) => {
    ...
    console.log(
      `worker ${worker.process.pid} disconnected a new one will be created once it exits`,
    );
  });
  ...
  cluster.on("exit", (worker: Worker, exitCode: number, signalCode: string) => {
    ...
    cluster.fork();
    console.log(
      `worker ${worker.process.pid} is attempting to signal for a new worker to take it's place`,
    );
  });
} else {
